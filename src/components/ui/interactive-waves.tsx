...
const Waves = ({ lineColor = '#C49A6C' }: WavesProps) => {
  ...
  useEffect(() => {
    ...
    const movePoints = (time: number) => {
      ...
      lines.forEach((points) => {
        points.forEach((p) => {
          const noiseX = (p.x + time * WAVE_TIME_X_FACTOR) * WAVE_NOISE_X_FACTOR;
          const noiseY = (p.y + time * WAVE_TIME_Y_FACTOR) * WAVE_NOISE_Y_FACTOR;
          const move = noise.perlin2(noiseX, noiseY) * WAVE_NOISE_MAGNITUDE;
    ...
    const tick = (time: number) => {
      ...
      mouse.vs += (mouse.v - mouse.vs) * MOUSE_SMOOTHING_FACTOR;
