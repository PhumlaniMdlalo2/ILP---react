  ...
  "relative flex flex-col justify-between h-full w-full overflow-hidden rounded-2xl p-8 shadow-sm transition-shadow duration-300 hover:shadow-lg",
  {
    variants: {
      gradient: {
        ...
        lime:   "bg-gradient-to-br from-lime-100 to-green-200/50",
      },
    },
...
const GradientCard = React.forwardRef<HTMLDivElement, GradientCardProps>(
  ({ className, gradient, badgeText, badgeColor, title, description, ctaText, ctaHref, ...props }, ref) => {
    const cardAnimation = {
      rest:  { scale: 1, y: 0 },
      ...
      >
        <div className={cn(cardVariants({ gradient }), className)} {...props}>
          <div className="flex flex-col h-full">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-white/60 px-3 py-1 text-sm font-medium text-gray-700 backdrop-blur-sm w-fit">
            ...
            <a
              href={ctaHref}
              className="group mt-6 inline-flex items-center gap-2 text-sm font-semibold text-gray-800"
            >
              {ctaText}
